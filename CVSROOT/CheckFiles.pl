...
$SEARCHPATTERN = "generated.*by (configure|automake|autoheader|.*createMakefile)"; # If the file contains this string, 
...
while ($#ARGV >= 0) {
	...
	while (<FILE>) {
	    if (/$SEARCHPATTERN/i) {
		print "ERROR! $directory/$file is a generated file and must not be commited!\n";
		$error = 1;
	       last;
	    }
	    $lines++;

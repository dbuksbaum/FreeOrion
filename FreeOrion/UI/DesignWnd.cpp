...
namespace {
    ...
    std::string LabelForShipSlotType(ShipSlotType slot_type) {
        switch (slot_type) {
        case SL_EXTERNAL:
            return UserString("SL_EXTERNAL");
        case SL_INTERNAL:
            return UserString("SL_INTERNAL");
        default:
            return "???";
        }
    }
...
void DesignWnd::SetDesignHull(const std::string& hull) {
    ...
    while (static_cast<unsigned int>(m_parts_lists.size()) < num_slots) {
        ...
        const std::string DROPLIST_LABEL_TEXT = LabelForShipSlotType(slot_type);
